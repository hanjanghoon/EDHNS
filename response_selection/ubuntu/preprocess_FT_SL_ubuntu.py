...
def search_in_subset(all_response, query):
    ...
    # 부분 집합에서 검색
    L,D,I =index_gpu.range_search(query[np.newaxis,:], thresh=0.001)
    ...
    if length<200:
        compensate_I=I[start:end].tolist()+random.sample(range(candidate_length),200)
...
def main():
    ...
    parser.add_argument("--in_file", type=str, default="ubuntu_data/train.pkl",help="Path to dataset.")
    parser.add_argument("--out_dir", type=str, default="new_data/",help="Path to dataset.")
    ...
    with open(args.out_dir+"train_SNDS_1000_0001.jsonl" , encoding= "utf-8", mode="w") as jsonl_file:
    ...
    with open(args.out_dir+"train_res_1000_0001.pkl", 'wb') as f:
