...
class Metric:
    ...
    def open_label(self, args):
        
        path = os.path.join(os.path.abspath(args.dataroot))
        
    
        labels_file = os.path.join(path, args.dataset, 'labels.json')
...
def main(argv):
    ...
    parser.add_argument('--dataset', default='test', dest='dataset', action='store', metavar='DATASET', choices=['train', 'val', 'test'], required=True, help='The dataset to analyze')
    parser.add_argument('--dataroot', default="dstc9_data", dest='dataroot',action='store', metavar='PATH', required=True,
                        help='Will look for corpus in <dataroot>/<dataset>/...')
    parser.add_argument('--outfile',default='runs/temp/pred.json',dest='outfile',action='store',metavar='JSON_FILE',required=True,
                        help='File containing output JSON')
    parser.add_argument('--scorefile', default='temp_score',dest='scorefile',action='store',metavar='JSON_FILE',required=True,
    ...
    labels = metric.open_label(args)
    preds = metric.open_pred(args)
    # jang
