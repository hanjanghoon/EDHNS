            ...
            if self.args.negative_sample_method == "random":
                ...
                if self.negative is None:
                    ...
                    if selected_negative is not None:
                        #확률 샘플링하고 하기.
                        candidate_keys=list(selected_negative)[:self.args.negative_num+1]
                        # candidate_keys =list(negative_list)
                        # 사실상 필요없음.
                        if example["knowledge_key"] not in candidate_keys:
                            candidate_keys=candidate_keys[:-1]+[example["knowledge_key"]]
