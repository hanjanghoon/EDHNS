...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, split_type, labels=True, labels_file=None):
        ...
        self.knowledge, self.raw_snippets, self.snippets = self._prepare_knowledge()
        
        # if (split_type=='ranking' or split_type=='train') and args.negative_sample_method=='ranking' :
        #     with open(self.dataroot+'/confined_negative3.json', 'r') as f:
        #         confined_candidates = json.load(f)
        if (split_type=='ranking') and args.negative_sample_method=='ranking' :
            with open(self.dataroot+'/sbert_negative045.json', 'r') as f:
                confined_candidates = json.load(f)
            # with open(self.dataroot+'/bm25_negative45.json', 'r') as f:
            #     confined_candidates = json.load(f)
            self.sbert_cand=confined_candidates['sbert']
    ...
    def _prepare_knowledge(self):
        ...
        return knowledge_all, raw_snippets, tokenized_snippets
...
class KnowledgeSelectionDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split_type == "val":
            ...
            if self.args.negative_sample_method == "random":
                ...
                if self.negative is None:
                    candidate_keys = list(self.snippets.keys())
                    # #bm25
                    # pool_keys=self.bm25_cand[knowledge_key][:10]
                    # candidate_keys = random.sample(list(pool_keys),min(self.args.negative_num, len(list(pool_keys))))
                    # candidate_keys=[knowledge_key]+candidate_keys
                    ...
                    if selected_negative is not None:
                        ...
                        #randomness x
                        negative_list=list(selected_negative)[:self.args.negative_num]
                        
                        #randomness 10
                        # negative_list=random.sample(list(selected_negative)[:10],self.args.negative_num+1)
                        
                        #debug
                        # random_neg=random.sample(list(self.snippets.keys()),1)[0]
                        # while random_neg !=example["knowledge_key"]:
                        #     random_neg=random.sample(list(self.snippets.keys()),1)[0]
                        


                        candidate_keys = list(negative_list)
                        candidate_keys=[example["knowledge_key"]]+candidate_keys


                        #debug
                        # candidate_keys=candidate_keys[:-1]+[random_neg]
                    else:
                        candidate_keys = list(self.snippets.keys())
                        
                        #bm25
                        # pool_keys=self.bm25_cand[knowledge_key]
                        #sbert
                        # pool_keys=self.sbert_cand[knowledge_key]
                        # candidate_keys = random.sample(list(pool_keys),min(self.args.negative_num, len(list(pool_keys))))
                        # candidate_keys=[knowledge_key]+candidate_keys
        ...
        #debug
            ...
            #ver original
            # bm25
            # pool_keys=self.bm25_cand[knowledge_key][:100]
            # pool_keys=self.bm25_cand[knowledge_key]
            #sbert
            # pool_keys=self.sbert_cand[knowledge_key][:500]
            # pool_keys=self.sbert_cand[knowledge_key]
            
            # pool_keys=self.bm25_cand[str(index)]       
            pool_keys=self.sbert_cand[str(index)]

            # candidate_keys=pool_keys
            # candidate_keys=pool_keys[:self.args.candidate_num]
            candidate_keys = random.sample(list(pool_keys),min(self.args.candidate_num, len(list(pool_keys) )))
